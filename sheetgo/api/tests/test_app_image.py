...
def test_should_get_error_if_no_file_is_sent(api_client):
    response = api_client.post('/image/convert')
...
def test_should_not_load_empty_file(api_client, empty_file):
    ...
    response = api_client.post('/image/convert', data=data)
    assert response.status_code == HTTPStatus.BAD_REQUEST
...
def test_should_not_load_file_with_invalid_request_format(api_client, invalid_file_ext):
    data = dict(file=(BytesIO(invalid_file_ext), 'file.wrong_ext'),)
...
def test_should_convert_image(api_client, simple_jpg):
...
def test_should_not_convert_image(injector, api_client, image_service, simple_jpg):
